...
How can Virginia prioritize which bridges get fixed? Is it simply a matter of looking at the lowest sufficiency rating. Or should we look at the probability of lives potentially being lossed given a low sufficiency rating? One way to answer this latter question is to analyze the daily vehicle traffic crossing each of the deficient bridges. 
...
The above chart was surprising because one would expect a "newly" constructed bridge to have sufficiency ratings eligible to receive replacement or rehabilitation funds. Clearly, this is not the case in Virginia. Where are these deficient bridges located? We will identify those jurisdictions with bridges constructed after the year 2000 and have sufficiency ratings less than 80. Because there are 74 jurisdictions meeting this criteria, we only list those with 8 or more bridges meeting this criteria.
...
Several methodologies exist to help prioritize which bridges get restored or replaced first. Clearly, one approach is to base prioritization on lowest sufficiency ratings (i.e., fixes the bridges that are in the worse condition). A good first cut. However, what if a bridge that has a median-level sufficiency rating supports traffic in an area where an increasing in population and resulting commuter traffic has a high traffic load. This bridge's condition would decline more rapidly, and if collapses, have a large economic, not to mention mortality impact than a really bad bridge serving a very rural population. 

Given the latter argument, we'll synthesize population and commuter data with bridge sufficiency ratings. 

#### Population Centers and Growth

Where are the largest Virginia population centers? Which ones are have a trending growing population? Perhaps these are the counties upon which to focus - give that they have bridges with sufficiency ratings indicating rehabilitation or replacement. 
...
ggplot(virginia.pop, aes(periodyear, population, group=areaname)) + 
  geom_line() +
  labs(x="Year", y="Population", title = "Annual Growth of Virginia Counties (1970-2012")
```

Clearly, most of Virginia's counties have relatively stable (one might say stagnant) population level. However, there five counties with population growing trends whose populations exceeded 300,000 by 2010. They are:
