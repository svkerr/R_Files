...
* Desire to use a _linear model_: Powerful notion that we can express the effect of the predictors on the response through a linear predictor
...
* Basic idea of Logistic Regression is to use linear regression by modeling the probability ($P_i$) using a linear predictor function.
...
We are interested in the probability of failure in O-rings is related to temperature, and the probability of failure at a given temperature. Oddly, we'll code a failure (extensive damage) = success (easier for modeling).
...
Now let's create a logistic regression model using this data:
```{r,echo=FALSE,message=FALSE}
...
summary(logitmod)
```     
We can now use the model to calculate:
